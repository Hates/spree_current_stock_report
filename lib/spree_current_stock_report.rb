require 'spree_core'
require 'spree_current_stock_report/engine'

class SpreeCurrentStockReport

  START_DATE = Date.parse("1/8/2013")
  ...
  def report
    ...
    totals = {}
    line_items = orders(created_at_gt: START_DATE.to_s(:sql)).collect(&:line_items).flatten
    ...
    thirty_day_totals = {}
    line_items = orders(created_at_gt: 30.days.ago.to_s(:sql)).collect(&:line_items).flatten
    line_items.each do |line_item|
      next if line_item.variant.deleted?
